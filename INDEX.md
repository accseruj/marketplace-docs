...
Living docs carry no history; `60-decisions/` is the history; git is the version record. Retirement rules and the hygiene checklist are in `CONVENTIONS.md`. Run `python3 scripts/docs-check.py` at every phase gate — it also runs in CI on push, PR and weekly, alongside `scripts/queue-check.py` and every test suite in `scripts/tests/`. Counting them here was already wrong once; the workflow is the list.
...
- INV-03 Every state transition in the value chain is writable by an API or event, never only by a human in a UI. Derived in `00-product/automation-charter.md` - readiness test #3 and anti-pattern #1. Revisit trigger: a value-chain stage where an API-writable transition proves impossible, or the charter's readiness test #3 being relaxed. OPEN: the charter is a living doc, so this derivation can move without an ADR - `bd` issue docs-muc.
