...
- The agent is read-only by rule, not by mechanism. `Write` and `Edit` are omitted from its tool list, but `Bash` is unscoped and can write, so the omission is a signal rather than a barrier. The definition must say so plainly. A `PreToolUse` deny-hook is the real instrument and is deliberately out of scope here.
